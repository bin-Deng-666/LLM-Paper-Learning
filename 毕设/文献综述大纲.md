...
介绍随着视觉语言模型的发展，对抗攻击的重点逐渐从早期的视觉模型发展至如今的视觉语言模型；

介绍面向VLMs的对抗攻击的定义，然后介绍几种主流方法；
...
在这一背景下，一系列针对视觉语言模型的对抗攻击研究应运而生。这些研究揭示了多模态系统的潜在安全风险，并为构建更加健壮的模型提供了重要见解。例如，Carlini等人采用了与早期原理相同的对抗样本生成技术，通过对抗图像扰动最大化模型输出有害文本的概率，指出大语言模型和多模态模型在面对对抗攻击时可能违反设计原则生成有害文本。Qi等人专注于视觉语言模型的越狱攻击，通过计算图像对抗扰动，使攻击能够绕过模型的安全防护，迫使模型执行原本会被拒绝的有害指令。Bagdasaryan的工作则强调了通过在图像的特定部分嵌入对抗扰动来操纵模型输出的可能性，这种方法能够在不显著改变图像整体语义内容的前提下，引导模型产生攻击者指定的文本。Schlarmann等人提出了非定向攻击，通过降低正确文本输出的概率来计算对抗扰动。然而，由于图像的正确描述可能有无穷多种，即使对抗扰动成功避免了一种正确描述，模型仍有可能生成另一种符合图像的正确描述，这在一定程度上削弱了攻击的效果。Bailey等人在攻击效果方面提出了“图像劫持”的概念，介绍了一种新的行为匹配算法来训练这些图像劫持，而且还展示了如何利用这种技术实施多种攻击，包括特定字符串攻击、泄露上下文攻击、越狱攻击和“幻觉”攻击。

尽管对抗样本的存在引发了人们对机器学习系统稳健性和可靠性的担忧，但研究人员发现了一个更有趣的现象：对抗样本的可转移性。可转移性指的是在一个模型上生成的对抗样本能够成功误导其他不同架构的模型。有了这种特性，就可以在不接触目标模型的模型架构和参数的情况下成功实施攻击，这为对抗攻击的研究和防御提供了新的视角。
